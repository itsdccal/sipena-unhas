<x-app-layout>
    ...
    <div class="inline-flex rounded-md shadow-xs mb-5 shadow-sm" role="group">
        ...
        <a href="{{ route('admin.degrees.index') }}" class="px-4 py-2 text-sm font-medium cursor-pointer text-gray-500 bg-gray-200 border-t border-b border-gray-200 hover:bg-white hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700">
        Jenjang
        </a>
        <a href="{{ route('admin.study-programs.index') }}" class="px-4 py-2 text-sm font-medium cursor-pointer text-gray-500 bg-gray-200 border border-gray-200 rounded-e-lg hover:bg-white hover:text-blue-700 focus:z-10 focus:ring-2 focus:ring-blue-700 focus:text-blue-700">
        Program Studi
        </a>
    </div>
                    ...
                    <form method="GET" action="{{ route('admin.faculties.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <input type="text" id="search" name="search" value="{{ request('search') }}"
                                placeholder="Cari berdasarkan kode dan nama fakultas ..."
                                ...
                                @foreach($faculties as $faculty)
                                    <tr class="hover:bg-gray-50">
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm font-medium text-gray-900">{{ $faculty->faculty_code }}</div>
                                        </td>
                                        <td class="px-6 py-4">
                                            <div class="text-sm text-gray-900">{{ $faculty->faculty_name }}</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap">
                                            <div class="text-sm text-gray-900">{{ $faculty->studyPrograms->count() }} program studi</div>
                                        </td>
                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
                                            <div class="flex items-center gap-2">
                                                <a href="{{ route('admin.faculties.edit', $faculty) }}" class="text-blue-600 hover:text-blue-900">
                                                    Edit
                                                </a>
                                                @if($faculty->studyPrograms->count() == 0 && $faculty->studyPrograms->count() == 0)
                                                    <form method="POST" action="{{ route('admin.faculties.destroy', $faculty) }}"
                                                    onsubmit="return confirm('Are you sure you want to delete this study faculty?');"
                                                    class="inline">
                                                    @csrf
                                                    @method('DELETE')
